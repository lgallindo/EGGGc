...
goodness.fit <-
  function(pdf, cdf, starts, data, method = "L-BFGS-B", domain = c(0,Inf),
           mle = NULL, lower=c(1e-10,1e-10,1e-10,1e-10), upper=c(Inf,Inf,Inf,Inf,Inf), control = list(type = 2), debug=TRUE){
    ...
    if(!is.null(lower)||!is.null(upper)) {
      print("Optimization bounds found. Defaulting to L-BFGS-B.")
      flush.console()
      method = "L"
    }
    ...
    if(is.null(mle)==TRUE){
      ...
      if (debug) {
        cat("MLE nulo. Construindo função de verossimilhança.\n")
        flush.console()
      }
      likelihood <- function(par, x){
        ...
        retval<--(log(product))
        if (debug) {
          cat("Função de log-verossimilhança chamada.")
          cat("par")
          print(par)
          cat("prod(pdf)")
          print(product)
      ...
      if (debug) {
        warnings()
        cat("Entrando no passo de otimização.\n")
        flush.console()
      }
